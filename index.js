...
app.post("/api/slider-images", upload.single('image'), async (req, res) => {
  try {
    ...
    const db = await connectToDatabase();
    const result = await db.collection("slider_images").insertOne({ imageUrl: req.file.path, createdAt: new Date() });
    res.status(201).json({ success: true, _id: result.insertedId, imageUrl: req.file.path });
...
app.get("/views", async (req, res) => {
  try {
    const db = await connectToDatabase();
    const result = await db.collection("site_views").findOne({ identifier: "total_views" });
